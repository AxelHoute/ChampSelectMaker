...
def getgame(elo,api_key): 
    ...
    for joueur in joueurs :
        if request<99 :
            try :
                game=requests.get(f'https://euw1.api.riotgames.com/lol/match/v5/matches/by-puuid/{joueur["puuid"]}/ids?start=0&count=15',headers=headers).json()
                print(game)
                games+=game
                request+=1
            except Exception as e  :
...
def getmatchs(games,api_key,elo,donnees):
    ...
    feature=['puuid','championId',"deaths","kills","damageDealtToObjectives","damageDealtToBuildings","magicDamageDealt","magicDamageTaken","physicalDamageDealt","physicalDamageTaken",
             "timeCCingOthers","totalDamageShieldedOnTeammates","totalHeal","goldSpent","goldEarned","totalEnemyJungleMinionsKilled","totalAllyJungleMinionsKilled","role","killingSprees","totalMinionsKilled"]
    ...
    for x in games:
        if request<99 :
            try :
                matchs= fetch(requests.get(f'https://euw1.api.riotgames.com/lol/match/v5/matches/{x}',headers=headers).json(),matchs,feature,feature_chall,feature_teams)
                request+=1
